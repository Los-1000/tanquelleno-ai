...
# -----------------------------------------------------------------------------
# 3. CANDIDATOS (Modelos Parsimoniosos y Estadísticamente Plausibles)
# -----------------------------------------------------------------------------
def construir_candidatos() -> dict:
    """Modelos parsimoniosos para series cortas (80 meses).

    Se descartan explícitamente modelos de ensamble de árboles complejos
    (boosting, random forest) y redes neuronales profundas que sobreajustan de
    forma severa sobre 80 observaciones.

    Se compara la Regresión Logística Regularizada (L2 Ridge) frente a su versión
    con calibración sigmoide de probabilidades (Platt Scaling).
    `class_weight="balanced"` es indispensable para contrarrestar el desbalance
    evidenciado en H2.
    """
    base_pipe = Pipeline(
        [
    ...
    return {
        "logistica_regularizada": base_pipe,
        "logistica_calibrada_platt": CalibratedClassifierCV(
            estimator=base_pipe,
            method="sigmoid",
            cv=3,
        ),

...
| **Fase M (Modelado & Despliegue)** | S7 – S11 | 🟡 **En curso** | Pipeline de entrenamiento ejecutable, modelo A2 evaluado sobre test ciego, modelo geoespacial de posición, pipeline end-to-end y auditoría del KR2. Pendientes: ingesta macro, integración LLM y **despliegue en Streamlit Cloud en Semana 10**. Resultados en [`analisis/fase_m_resultados.md`](analisis/fase_m_resultados.md). |
...
make posicion     # entrena el modelo geoespacial de posición de precio
make inferir      # ejecuta el pipeline end-to-end y muestra el payload
make auditoria    # mide el KR2 sobre 60 casos
make pruebas      # corre las 22 pruebas del contrato metodológico
make todo         # la cadena completa
```
...
### 6.3 Variables macroeconómicas (paso pendiente)
...
Cifras regeneradas por `make todo`, no declarativas. Detalle e interpretación en [`analisis/fase_m_resultados.md`](analisis/fase_m_resultados.md).

| KR | Meta | Medido | Estado |
|:---:|---|---|:---:|
| **KR1** · Macro-F1 en test ciego 2026 | ≥ 0.55 | 0.4418 con argmax · 0.2058 con P\*=0.25 | ❌ No alcanzada (baseline: 0.0833) |
| **KR1** · Recall en alzas | ≥ 70% | **93.8%** | ✅ Cumple |
| **KR2** · Recomendaciones sin alucinación | 100% (N ≥ 50) | **100%** sobre 60 casos | ✅ Cumple |
| **KR3** · Despliegue en la nube | 100% en S10 | — | ⚪ Semana 10 |
| **KR4** · Ahorro por carga | ≥ S/ 8.00 | **S/ 9.60** medianos en radio de 2 km | ✅ Cumple |

Hallazgo destacado: el descuento de aplicativo de Repsol equivale a **S/ 24.00 por tanqueada**, más que el ahorro por elegir bien el grifo y más que el ahorro por acertar la tendencia mensual.
